...
using namespace ValueB;

static void _registerBots() {
  registerBotFactory("ValueBot", std::shared_ptr<Hanabi::BotFactory>(new ::BotFactory<ValueBot>()));
}
...
void ValueBot::invalidateKnol(int player_index, int card_index)
{
    ...
    std::vector<ValueB::CardKnowledge> &vec = handKnowledge_[player_index];
...
void ValueBot::wipeOutPlayables(const Hanabi::Server &server, const Card &played_card)
{
    ...
    for (int player = 0; player < numPlayers; ++player) {
        for (int c = 0; c < server.sizeOfHandOfPlayer(player); ++c) {
            ValueB::CardKnowledge &knol = handKnowledge_[player][c];
...
ValueB::Hint ValueBot::bestHintForPlayer(const Server &server, int partner) const
{
    assert(partner != me_);
    ...
    for (Color color = RED; color <= BLUE; ++color) {
        ...
        for (int c=0; c < partners_hand.size(); ++c) {
            const ValueB::CardKnowledge &knol = handKnowledge_[partner][c];
...
bool ValueBot::maybeGiveHelpfulHint(Server &server)
{
    ...
    const int numPlayers = handKnowledge_.size();
    ValueB::Hint bestHint;
    for (int i = 1; i < numPlayers; ++i) {
        const int partner = (me_ + i) % numPlayers;
        ValueB::Hint candidate = bestHintForPlayer(server, partner);
